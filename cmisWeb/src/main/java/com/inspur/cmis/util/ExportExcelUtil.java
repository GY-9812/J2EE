...
public class ExportExcelUtil {
	 ...
	 */
	public static XSSFWorkbook createExcelFile(Class<?> clazz,
			List<HashMap<String, Object>> objs, Map<Integer, List<ExcelBean>> map,
	...
	private static void createTableRows(
			XSSFSheet sheet, 
			Map<Integer,List<ExcelBean>> map, 
			List<HashMap<String,Object>> objs, 
			Class<?> clazz) 
		...
		for(HashMap<String,Object> obj : objs){
			...
			for(int i=0;i<ems.size();i++){
				  ...
				  String cellValue = "";
				  if("sex".equals(propertyName)){
					  cellValue = value.equals("F")?"女":"男";
				  }else if(value==null){
					  cellValue = "";
